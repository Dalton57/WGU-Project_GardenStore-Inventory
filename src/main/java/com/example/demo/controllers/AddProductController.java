...
//        product.getParts().addAll(assparts);
            ...
            if (product.getId() != 0) {
                ...
                int invChange = product.getInv() - product2.getInv();

                if (invChange > 0) {
                    for (Part p : product2.getParts()) {
                        if (p.getInv() - invChange < p.getMinInv()) {
                            bindingResult.reject("error.product", "Not enough inventory for part " + p.getName() + ", Minimum required: " + p.getMinInv());

                            theModel.addAttribute("parts", partService.findAll());
                            theModel.addAttribute("availparts", partService.findAll());
                            theModel.addAttribute("asspart", product2.getParts());

                            return "productForm";
                        }
                    }
                    for (Part p : product2.getParts()) {
                        p.setInv(p.getInv() - invChange);
                        partService1.save(p);
                    }
                }
                else {
